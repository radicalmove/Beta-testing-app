...
class DeploymentPackageTests(unittest.TestCase):
    ...
    def test_guides_cover_tailscale_restore_and_named_pilot_targets(self):
        ...
        for token in ("0.5.4", "pilot-builds/moodle-review-extension", "chrome://extensions", "edge://extensions", "exactly one teal review toolbar", "Clear all", "revoke the permission"):
            self.assertIn(token, pilot)
        for route in (
