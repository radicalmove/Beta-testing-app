...
The server resume endpoint verifies in this order: find the course membership by normalized email and course, find its active reconnect credential, verify the submitted credential hash, then inspect membership state. Any missing course/email/membership/credential, invalid credential, or mismatch returns the same generic HTTP 403 response and discloses no state. Only after successful credential verification does it return HTTP 200 with `state: "pending"`, `"rejected"`, or `"revoked"`; approved returns HTTP 200 with `state: "approved"`, the normal session token, device credential, and expiry. Terminal responses therefore cannot be used as an email/membership oracle.

Changing a membership to rejected or revoked does not immediately revoke its reconnect verifier. The verifier remains usable only to authenticate this terminal-state query until the earlier of its existing expiry or 30 days after the membership transition. Terminal membership state is checked before any access issuance, so this verifier can never create a session or device credential. A scheduled/lazy cleanup deletes expired terminal verifiers. An administrative membership reset deletes the membership and its verifier together.

For a locally stored pending record, a generic HTTP 403 means the credential is no longer usable (invalidated, deleted, administratively reset, or mismatched). The background deletes that local record and returns `none`; the UI shows **This invitation is no longer valid — ask the course team for a new invitation.** Network failures, timeouts, HTTP 429, and HTTP 5xx retain the record and retry. Route, service, and background tests cover every state, the validation order, terminal-verifier expiry, generic 403 cleanup, administrative reset, and credential deletion.
