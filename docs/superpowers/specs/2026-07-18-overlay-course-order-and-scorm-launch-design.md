...
- The trusted top-frame document supplies the positive-integer Moodle course-module id using the existing `explicitActivityIdFromDocument` sources: `body[data-cmid]`, `body[data-activityid]`, or one unique `cmid-<number>` body class. The content script fetches the same-origin authenticated `/mod/scorm/view.php?id=<cmid>` document and accepts exactly one POST form whose action has the exact `/mod/scorm/player.php` pathname and no credentials.
...
- The extension also stores the mapping in `chrome.storage.session`, so it survives extension service-worker restarts but not a browser-session boundary. A mapping record contains the resolved course UUID, configured Moodle origin, positive cmid, exact package-root URL, complete validated player URL, creation time, and expiry time. The package-root URL is the canonical HTTPS worker `page_url` prefix from its origin through the first `/scormcontent/` path segment, with query and fragment removed; URLs without exactly one trusted `pluginfile.php/.../mod_scorm/.../scormcontent/` root are not cacheable. The key is the course UUID plus the exact package-root URL. Records expire after 12 hours, the collection is limited to 128 records, and overflow evicts the oldest creation time. Lookup requires exact course UUID, configured origin, and package root; expired, malformed, origin-mismatched, course-mismatched, or player/cmid-inconsistent records are removed. A newly validated visit replaces the exact key. This allows older comments with the same package identity and a bare player parent to be upgraded locally before navigation without cross-course or stale matching.
