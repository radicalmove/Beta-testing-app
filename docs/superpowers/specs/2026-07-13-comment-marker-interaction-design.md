...
Keyboard placement uses normal document focus order rather than inventing a second navigation system. On entering placement mode, focus returns to the most recently focused eligible page element, or the first eligible focusable/content landmark after Moodle navigation. Tab and Shift+Tab move normally. A turquoise preview marker follows the currently focused eligible target; Enter or Space places the marker at that element's logical top-right inset. Escape cancels. Placement instructions and target changes are announced through the overlay status region.
