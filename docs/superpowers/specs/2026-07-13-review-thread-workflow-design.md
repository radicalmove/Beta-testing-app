...
- **Switch user** clears the active API session and in-memory identity/context, then opens the course-scoped saved-reviewer chooser. It retains each approved reviewer's rotating device credential in trusted extension storage, keyed by course and reviewer, so choosing a saved reviewer can authenticate automatically. **Forget this reviewer** explicitly removes that reviewer's device credential and non-secret label. Pending/reconnect credentials are cleared when their onboarding flow completes or is abandoned.
