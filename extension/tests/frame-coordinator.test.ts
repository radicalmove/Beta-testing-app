...
test("elects deepest stable content frame", () => {
  ...
  coordinator.registerCapabilities(1, 0, "document-0", 1, workerA, content(1_000_000), 0);
  coordinator.registerCapabilities(1, 2, "document-2", 1, workerA, { ...content(), wrapper: true }, 0);
  coordinator.registerCapabilities(1, 7, "document-7", 1, workerA, content(), 0);
...
test("breaks sibling ties by visible area then stable frame id", () => {
  ...
  coordinator.registerCapabilities(1, 8, "document-8", 1, workerA, content(400), 0);
  coordinator.registerCapabilities(1, 4, "document-4", 1, workerA, content(900), 0);
  assert.equal(coordinator.advanceElection(1, 0).candidateFrameId, 4);

  coordinator.registerCapabilities(1, 8, "document-8", 1, workerA, content(900), 1);
...
test("never reports two active frames during acknowledged handover", () => {
  ...
  coordinator.bindCourse(1, "course-a", 0);
  coordinator.registerNavigation(1, 0, -1, "https://moodle.example/course", "document-0");
  coordinator.registerCapabilities(1, 0, "document-0", 1, workerA, content(), 0);
  ...
  coordinator.registerCapabilities(1, 3, "document-3", 1, workerB, content(), 1);
...
test("hidden owner chain makes a large content child ineligible", () => {
  ...
  coordinator.registerNavigation(1, 7, 2, "https://rise.example/lesson", "document-7");
  coordinator.registerCapabilities(1, 0, "document-0", 1, workerA, content(100), 0);
  coordinator.registerCapabilities(1, 2, "document-2", 1, workerA, { ...content(), wrapper: true }, 0);
  coordinator.registerCapabilities(1, 7, "document-7", 1, workerA, content(1_000_000), 0);
...
test("authoritative frame removal permits a pending replacement", () => {
  ...
  coordinator.registerCapabilities(1, 0, "document-0", 1, workerA, { ...content(), wrapper: true }, 0);
  coordinator.registerCapabilities(1, 2, "document-2", 1, workerA, content(), 0);
  const first = coordinator.advanceElection(1, 0);
  coordinator.confirmActivated(1, 2, workerA, first.generation!);
  coordinator.registerNavigation(1, 3, 0, "https://rise.example/lesson", "document-3");
  coordinator.registerCapabilities(1, 3, "document-3", 1, workerB, content(2_000_000), 0);
...
test("rebinding the same course preserves frames while tab removal clears them", () => {
  ...
  coordinator.registerNavigation(1, 0, -1, "https://moodle.example/course", "document-0");
  coordinator.registerNavigation(1, 3, 0, "https://rise.example/lesson", "document-3");
  coordinator.registerCapabilities(1, 3, "document-3", 1, workerA, content(), 0);
  coordinator.bindCourse(1, "course-a", 0);
...
test("a new worker instance on the same frame clears stale ownership and stability", () => {
  ...
  coordinator.registerCapabilities(1, 7, "document-7", 2, workerB, content(), 1);
  assert.deepEqual(coordinator.snapshot(1).activeFrameIds, []);
  const replacement = coordinator.advanceElection(1, 1);
  ...
  assert.equal(coordinator.registerCapabilities(1, 7, "document-7", 1, workerA, content(), 2), false);
  assert.equal(coordinator.registerCapabilities(1, 7, "document-7", 2, workerC, content(), 2), false);
  assert.equal(coordinator.advanceElection(1, 2).candidateWorkerInstanceId, workerB);

  assert.equal(coordinator.registerCapabilities(1, 7, "document-7", 3, workerC, content(), 3), true);
  assert.equal(coordinator.advanceElection(1, 3).activateWorkerInstanceId, workerC);
  for (const invalidEpoch of [-1, 0, 1.5, Number.MAX_SAFE_INTEGER]) {
    assert.equal(coordinator.registerCapabilities(1, 7, "document-7", invalidEpoch, workerA, content(), 4), false);
  }
});
...
test("a stale same-frame document cannot reclaim ownership after its replacement", () => {
  ...
  coordinator.registerCapabilities(1, 7, "document-a", 1, workerA, content(), 0);
  const first = coordinator.advanceElection(1, 0);
  ...
  coordinator.registerCapabilities(1, 7, "document-b", 1, workerB, content(), 1);
  const second = coordinator.advanceElection(1, 1);
  coordinator.confirmActivated(1, 7, workerB, second.generation!);

  coordinator.registerCapabilities(1, 7, "document-a", 1, workerA, content(2_000_000), 2);
  ...
  coordinator.registerCapabilities(1, 7, "document-c", 1, workerC, content(), 3);
...
test("trusted document identity permits repeated same-URL replacements without a fixed ceiling", () => {
  ...
  coordinator.registerNavigation(1, 7, 0, "https://rise.example/lesson", "document-shared");
  for (const [index, worker] of workers.entries()) {
    coordinator.registerCapabilities(1, 7, "document-shared", index + 1, worker, content(), index);
...
test("authoritative navigation prunes departed frames so they cannot win", () => {
  ...
  coordinator.registerCapabilities(1, 9, "document-9", 1, workerB, content(100), 1);
