...
test("uses authoritative navigation parentage and activates the deepest frame", async () => {
  ...
  await runtime.registerFrame(1, 7, workerA, content, frames, 0);
...
test("deactivates and confirms dormancy before activating a replacement", async () => {
  ...
  runtime.bindCourse(1, "course-a");
  await runtime.registerFrame(1, 2, workerA, content, frames, 0);
  await runtime.registerFrame(1, 7, workerB, { ...content, area: 900_000 }, frames, 1);
...
test("activates only after the election stability window", async () => {
  ...
  runtime.bindCourse(1, "course-a");
  await runtime.registerFrame(1, 7, workerA, content, frames, 1000);
  assert.deepEqual(sent, []);
...
test("times out a hanging deactivation, ignores its late acknowledgement, and continues election", async () => {
  ...
  runtime.bindCourse(1, "course-a");
  await runtime.registerFrame(1, 2, workerA, content, frames, 0);
  const replacement = runtime.registerFrame(1, 7, workerB, { ...content, area: 900_000 }, frames, 1);
...
test("an abandoned worker instance cannot reclaim ownership through a stale lease", async () => {
  const siblingFrames = [
    { frameId: 0, parentFrameId: -1, url: "https://moodle.example/mod/scorm/player.php" },
    { frameId: 2, parentFrameId: 0, url: "https://rise.example/old" },
    { frameId: 7, parentFrameId: 0, url: "https://rise.example/current" },
  ];
  const sent: Array<{ frameId: number; type: string }> = [];
  ...
  runtime.bindCourse(1, "course-a");
  await runtime.registerFrame(1, 2, workerA, content, siblingFrames, 0);
  failDeactivation = true;
  await runtime.registerFrame(1, 7, workerB, { ...content, area: 900_000 }, siblingFrames, 1);
  await runtime.registerFrame(1, 2, workerA, { ...content, area: 2_000_000 }, siblingFrames, 2);
...
test("notifies worker readiness after replacement and after runtime reconstruction", async () => {
  ...
  await first.registerFrame(1, 7, workerA, content, frames, 0);
  await first.registerFrame(1, 7, workerB, content, frames, 1);
  ...
  await reconstructed.registerFrame(1, 7, workerB, content, frames, 2);
