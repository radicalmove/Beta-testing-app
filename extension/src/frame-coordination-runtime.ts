...
export class FrameCoordinatorRuntime {
  ...
  async registerFrame(tabId: number, frameId: number, workerInstanceId: string, capabilities: FrameCapabilities, navigation: NavigationFrame[], now = this.now()): Promise<void> {
    this.coordinator.replaceNavigation(tabId, navigation);
    this.coordinator.registerCapabilities(tabId, frameId, workerInstanceId, capabilities, now);
