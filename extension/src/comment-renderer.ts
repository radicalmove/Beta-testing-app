  ...
  const openThread = (comment: PageComment, index: number, marker: HTMLElement) => {
    ...
    if (comment.capabilities.can_edit && options.editThread) {
      ...
      edit.addEventListener("click", () => {
        ...
        const save = document.createElement("button"); save.type = "button"; save.dataset.saveEdit = "true"; save.className = "edit-save"; save.textContent = "Save";
        ...
        const actions = document.createElement("div"); actions.className = "edit-actions"; actions.dataset.editActions = "true"; actions.append(save, cancel);
        ...
        editor.append(input, attachmentLabel, actions); body.hidden = true; body.after(editor); edit.setAttribute("aria-pressed", "true"); input.focus();
      });
