...
function createController(host: HTMLElement, shadow: ShadowRoot, initial: CourseContext, initialStatus: ConnectionStatus, options: ReviewOverlayOptions, buildDiagnostics: BuildDiagnostics): ReviewOverlay {
  ...
  const openAccessDialog = (trigger: HTMLElement) => {
    ...
    backdrop.innerHTML = `<form class="dialog" data-access-form><h2>Join this course review</h2><label class="field">Name<input name="displayName" required maxlength="100"></label><label class="field">Role<select name="role"><option value="beta_tester">Beta tester</option><option value="sme">Subject matter expert</option><option value="ld_dcd">Learning designer / course developer</option></select></label><label class="field">Email<input name="email" type="email" required maxlength="320"></label><label class="field">Invitation code<input name="code" required maxlength="32" autocomplete="one-time-code"></label><p class="error" role="alert" hidden></p><div class="actions"><button type="button" data-cancel>Cancel</button><button type="submit" class="primary">Continue</button></div></form>`;
