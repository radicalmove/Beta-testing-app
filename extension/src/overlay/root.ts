...
function createController(host: HTMLElement, shadow: ShadowRoot, initial: CourseContext, initialStatus: ConnectionStatus, options: ReviewOverlayOptions, buildDiagnostics: BuildDiagnostics): ReviewOverlay {
  ...
  const openAccessDialog = (trigger: HTMLElement) => {
    ...
    backdrop.innerHTML = `<form class="dialog" data-access-form><h2>Join this course review</h2><div class="mode-tabs"><button type="button" data-mode="new" aria-pressed="true">New reviewer</button><button type="button" data-mode="existing" aria-pressed="false">Existing reviewer</button></div><div data-new-fields><label class="field">Name<input name="displayName" required maxlength="100"></label><label class="field">Role<select name="role"><option value="beta_tester">Beta tester</option><option value="sme">Subject matter expert</option><option value="ld_dcd">Learning designer / course developer</option></select></label></div><label class="field">Email<input name="email" type="email" required maxlength="320"></label><label class="field"><span data-code-label>Invitation code</span><input name="code" required maxlength="32" autocomplete="one-time-code"></label><p class="error" role="alert" hidden></p><div class="actions"><button type="button" data-cancel>Cancel</button><button type="submit" class="primary">Continue</button></div></form>`;
    let mode: "new" | "existing" = "new";
    const setMode = (next: "new" | "existing") => { mode = next; backdrop.querySelector<HTMLElement>("[data-new-fields]")!.hidden = next === "existing"; backdrop.querySelector<HTMLElement>("[data-code-label]")!.textContent = next === "new" ? "Invitation code" : "Personal reconnect code"; backdrop.querySelectorAll<HTMLElement>("[data-mode]").forEach((button) => button.setAttribute("aria-pressed", String(button.dataset.mode === next))); };
    backdrop.querySelectorAll<HTMLButtonElement>("[data-mode]").forEach((button) => button.addEventListener("click", () => setMode(button.dataset.mode as "new" | "existing")));
    const close = () => { backdrop.remove(); returnFocus?.focus(); };
    backdrop.querySelector("[data-cancel]")?.addEventListener("click", close);
    backdrop.querySelector("form")?.addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const data = new FormData(form); const error = form.querySelector<HTMLElement>(".error")!; const submit = form.querySelector<HTMLButtonElement>('button[type="submit"]')!; submit.disabled = true; error.hidden = true; try { const email = String(data.get("email") ?? "").trim(); const code = String(data.get("code") ?? "").trim(); const input: ReviewerAccessInput = mode === "new" ? { mode, displayName: String(data.get("displayName") ?? "").trim(), email, role: String(data.get("role") ?? "beta_tester"), code } : { mode, email, code }; const outcome = await options.onAccessSubmit!(input); if (outcome.reconnectCode) { form.innerHTML = `<h2>Save your reconnect code</h2><p>Keep this private. You need it only when using another browser.</p><output class="reconnect-code">${escapeHtml(outcome.reconnectCode)}</output><div class="actions"><button type="button" class="primary" data-done>Done</button></div>`; form.querySelector("[data-done]")?.addEventListener("click", () => { close(); status = outcome.status; renderStateControls(outcome.message); }); return; } close(); status = outcome.status; renderStateControls(outcome.message); } catch (caught) { error.textContent = caught instanceof Error ? caught.message : "Unable to verify reviewer access"; error.hidden = false; submit.disabled = false; } });
