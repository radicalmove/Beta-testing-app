...
import { saveIconMarkup } from "../ui/save-icon.ts";
...
export const semanticFilterHoverStyles = `.comment-filters .comment-scope:hover{background:var(--review-scope);border-color:var(--review-scope);color:#fff}.comment-filters .comment-status:hover{background:var(--review-status);border-color:var(--review-status);color:#fff}.comment-filters .comment-scope[aria-pressed="true"]:hover{background:#fff;border-color:var(--review-scope);color:var(--review-scope)}.comment-filters .comment-status[aria-pressed="true"]:hover{background:#fff;border-color:var(--review-status);color:var(--review-status)}.comment-jump[aria-expanded="true"]{background:var(--review-jump);border-color:var(--review-jump);color:#fff}.comment-page-field .comment-jump[aria-expanded="true"]:hover{background:#fff;border-color:var(--review-jump);color:var(--review-jump)}.toolbar-actions [data-action="add-comment"]{background:#fff;border-color:var(--review-dark-teal);color:var(--review-dark-teal)}.toolbar-actions [data-action="add-comment"]:hover{background:var(--review-dark-teal);border-color:var(--review-dark-teal);color:#fff}.toolbar-actions [data-action="add-comment"][aria-pressed="true"]{background:var(--review-red);border-color:var(--review-red);color:#fff}.toolbar-actions [data-action="add-comment"][aria-pressed="true"]:hover{background:#fff;border-color:var(--review-red);color:var(--review-red)}.comment-row-action.delete-action{border-radius:5px}.comment-row-action.delete-action:hover{background:#fff;border-color:var(--review-red)}.comment-row-action.delete-action:hover .delete-body{fill:var(--review-red)}.comment-row-action.delete-action:hover .delete-lines{stroke:#fff}.comment-page-list{top:auto;bottom:42px;width:min(380px,calc(100vw - 32px))}.comment-page-option{min-height:34px!important;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;font-weight:500}.comment-page-option[aria-selected="true"]{font-weight:650}.shell:has(.comment-jump[aria-expanded="true"]),.panel:has(.comment-jump[aria-expanded="true"]){overflow:visible}`;
...
function createController(host: HTMLElement, shadow: ShadowRoot, initial: CourseContext, initialStatus: ConnectionStatus, options: ReviewOverlayOptions, buildDiagnostics: BuildDiagnostics): ReviewOverlay {
  ...
  const deleteIcon = () => { const svg = ownerDocument.createElementNS("http://www.w3.org/2000/svg", "svg"); svg.setAttribute("viewBox", "0 0 24 24"); svg.setAttribute("aria-hidden", "true"); svg.innerHTML = '<path class="delete-body" d="M4 6h16l-1.4 15H5.4L4 6Z" fill="white"/><path class="delete-body" d="M8 3h8l1 2H7l1-2ZM3 5h18v2H3V5Z" fill="white"/><path class="delete-lines" d="M8.5 9v8M12 9v8M15.5 9v8" stroke="#d73b3d" stroke-width="1.8" stroke-linecap="round"/>'; return svg; };
  const openDialog = (trigger: HTMLElement, label: string, anchor: CommentAnchor, embeddedCapability?: string) => {
    ...
    backdrop.innerHTML = `<div class="dialog" role="dialog" aria-modal="true" aria-labelledby="review-dialog-title"><h2 id="review-dialog-title">${escapeHtml(label)}</h2><div class="preview">${escapeHtml(preview)}</div><div class="comment-composer"><div class="comment-composer-field-row"><textarea data-initial-focus required aria-label="Comment"></textarea><button type="button" class="comment-composer-save" data-save aria-label="Save comment" title="Save comment">${saveIconMarkup()}</button></div><label class="field">Attach a file (optional)<input type="file" data-attachment accept=".pdf,.doc,.docx,.png,.jpg,.jpeg,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/png,image/jpeg"><small>PDF, Word, PNG or JPEG; maximum 10 MB.</small></label><div class="comment-composer-actions"><button type="button" class="comment-composer-cancel" data-cancel>Cancel</button></div></div><div class="error" role="alert" hidden></div></div>`;
