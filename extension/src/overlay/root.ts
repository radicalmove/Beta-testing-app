...
export function mountReviewOverlay(document: Document, context: CourseContext, status: ConnectionStatus = "connecting", options: ReviewOverlayOptions & SignOutOptions = {}, buildDiagnostics: BuildDiagnostics = defaultBuildDiagnostics): ReviewOverlay {
  ...
  style.textContent += stabilisationUxStyles;
  ...
  const openAccessDialog = (trigger: HTMLElement) => {
    ...
    const setMode = async (mode: "new" | "existing") => {
      ...
      if (mode === "existing") {
        ...
        for (const reviewer of saved) { const option = ownerDocument.createElement("option"); option.value = reviewer.membershipId; option.textContent = reviewer.label; select.append(option); }
    ...
    backdrop.querySelector("[data-use-existing]")?.addEventListener("click", async () => { const membershipId = backdrop.querySelector<HTMLSelectElement>('[name="savedReviewer"]')!.value; if (!membershipId || !options.onUseSavedReviewer) return; const outcome = await options.onUseSavedReviewer(membershipId); close(); status = outcome.status; renderStateControls(outcome.message); });
