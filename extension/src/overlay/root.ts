    ...
    setCommentList(comments) {
      ...
      for (const [commentIndex, comment] of comments.entries()) {
        ...
        if (comment.capabilities?.can_change_status && options.changeStatus) {
          ...
          const action = ownerDocument.createElement("button"); action.type = "button"; action.className = "comment-row-action"; action.dataset.commentAction = "status"; action.textContent = nextStatus === "open" ? "↻" : "✓"; action.setAttribute("aria-label", `${verb} comment ${commentIndex + 1}`); action.title = `${verb} comment ${commentIndex + 1}`;
          action.disabled = rowMutations.get(mutationKey("status"))?.pending === true; action.addEventListener("click", () => void runRowMutation("status", () => options.changeStatus!(comment.id, nextStatus), `Could not ${verb.toLowerCase()} comment`)); rowActions.append(action);
        }
        if (comment.capabilities?.can_delete && options.deleteThread) {
          const action = ownerDocument.createElement("button"); action.type = "button"; action.className = "comment-row-action"; action.dataset.commentAction = "delete"; action.textContent = "🗑"; action.setAttribute("aria-label", `Delete comment ${commentIndex + 1}`); action.title = `Delete comment ${commentIndex + 1}`;
          action.disabled = rowMutations.get(mutationKey("delete"))?.pending === true; action.addEventListener("click", () => { if (ownerDocument.defaultView?.confirm && !ownerDocument.defaultView.confirm("Delete this entire thread, including all replies and screenshots?")) return; void runRowMutation("delete", () => options.deleteThread!(comment.id), "Could not delete comment"); }); rowActions.append(action);
        }
