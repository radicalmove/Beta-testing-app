...
function createController(host: HTMLElement, shadow: ShadowRoot, initial: CourseContext, initialStatus: ConnectionStatus, options: ReviewOverlayOptions, buildDiagnostics: BuildDiagnostics): ReviewOverlay {
  ...
  return {
    ...
    setPageComments(comments) {
      ...
      const applyFilter = () => { panelContent.querySelectorAll<HTMLElement>("[data-comment-item]").forEach((node) => { node.hidden = node.dataset.commentGroup !== activeFilter; }); filters.querySelectorAll<HTMLElement>("button").forEach((button) => button.setAttribute("aria-pressed", String(button.dataset.commentFilter === activeFilter))); };
      ...
      for (const [commentIndex, comment] of comments.entries()) {
        const openThread = (marker?: HTMLElement) => {
          ...
          if (comment.capabilities.can_change_status && options.changeStatus) { const target = comment.status === "resolved" ? "open" : "resolved"; const statusButton = ownerDocument.createElement("button"); statusButton.type = "button"; statusButton.className = `resolve-toggle${comment.status === "resolved" ? " resolved" : ""}`; statusButton.textContent = target === "resolved" ? "☐ Resolve" : "☑ Resolved"; statusButton.setAttribute("aria-label", target === "resolved" ? "Resolve this comment" : "Reopen this resolved comment"); statusButton.addEventListener("click", async () => { statusButton.disabled = true; try { await options.changeStatus!(comment.id, target); article.remove(); } catch (error) { statusButton.disabled = false; const alert = ownerDocument.createElement("p"); alert.setAttribute("role", "alert"); alert.textContent = error instanceof Error ? error.message : "Could not update status"; article.append(alert); } }); article.append(statusButton); }
        ...
        const item = ownerDocument.createElement("button"); item.type = "button"; item.className = "comment-index-link"; item.dataset.commentItem = comment.id; item.dataset.commentGroup = comment.status === "resolved" ? "resolved" : "open"; const shortPage = comment.page_title.replace(/\s+/g, " ").trim().slice(0, 32); const shortBody = comment.body.replace(/\s+/g, " ").trim().slice(0, 56); item.textContent = `#${commentIndex + 1} · ${shortPage} · “${shortBody}${comment.body.length > 56 ? "…" : ""}”`; item.setAttribute("aria-label", `Comment ${commentIndex + 1}. ${comment.page_title}. ${comment.body}. ${comment.author.display_name}. Status ${comment.status}.`); item.addEventListener("click", async () => { if (comment.page_url !== context.page_url) { if (options.navigateToComment) await options.navigateToComment(comment.id, comment.page_url); else ownerDocument.defaultView?.location.assign(comment.page_url); return; } panel.hidden = true; shadow.querySelector<HTMLElement>('[data-action="panel"]')?.setAttribute("aria-expanded", "false"); const marker = ownerDocument.querySelector<HTMLElement>(`[data-moodle-review-stored-highlight="${comment.id}"],[data-moodle-review-stored-pin="${comment.id}"]`); if (marker) { marker.scrollIntoView?.({ block: "center" }); marker.focus(); openThread(marker); } }); panelContent.append(item);
      }
    ...
    takeToContext(id) {
      ...
      if (target) { target.scrollIntoView?.({ block: "center" }); target.focus(); target.click(); return true; }
