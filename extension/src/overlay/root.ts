    ...
    setCommentList(comments) {
      ...
      for (const group of projection.groups) {
        ...
        if (comment.capabilities?.can_delete && options.deleteThread) {
          const action = ownerDocument.createElement("button"); action.type = "button"; action.className = "comment-row-action delete-action"; action.dataset.commentAction = "delete"; action.append(createReviewIcon(ownerDocument, "delete")); action.setAttribute("aria-label", `Delete comment ${commentIndex + 1}`); action.title = `Delete comment ${commentIndex + 1}`;
