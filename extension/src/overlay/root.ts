      ...
      for (const [commentIndex, comment] of comments.entries()) {
        const openThread = (marker?: HTMLElement) => {
          ...
          if (comment.capabilities.can_change_status && options.changeStatus) {
            ...
            const statusButton = ownerDocument.createElement("button"); statusButton.type = "button";
            statusButton.className = `resolve-toggle${comment.status === "resolved" ? " resolved" : ""}`;
            statusButton.style.cssText = "font-size:18px;min-height:48px;padding:8px 14px;background:#e9f7ee;color:#11652e;border:2px solid #16833b";
            statusButton.textContent = target === "resolved" ? "☐ Resolve" : "☑ Resolved";
            statusButton.setAttribute("aria-label", target === "resolved" ? "Resolve this comment" : "Reopen this resolved comment");
            statusButton.addEventListener("click", async () => {
              statusButton.disabled = true;
              try {
                await options.changeStatus!(comment.id, target);
                if (target === "resolved") {
                  statusButton.textContent = "☑ Resolved";
                  statusButton.classList.add("resolved");
                  statusButton.style.background = "#16833b"; statusButton.style.color = "#fff";
                  ownerDocument.defaultView?.setTimeout(() => article.remove(), 3000);
                } else article.remove();
              } catch (error) {
                statusButton.disabled = false; const alert = ownerDocument.createElement("p"); alert.setAttribute("role", "alert"); alert.textContent = error instanceof Error ? error.message : "Could not update status"; article.append(alert);
              }
            });
            article.append(statusButton);
          }
          ...
          if (marker) { const position = () => { const rect = marker.getBoundingClientRect(); const viewportWidth = ownerDocument.defaultView?.innerWidth ?? 800; const viewportHeight = ownerDocument.defaultView?.innerHeight ?? 600; article.hidden = rect.top < 0 || rect.left < 0 || rect.bottom > viewportHeight || rect.right > viewportWidth; if (article.hidden) return; const width = Math.min(360, viewportWidth - 16); const height = Math.min(article.offsetHeight || 300, viewportHeight - 16); const right = rect.right + 8; const left = right + width <= viewportWidth - 8 ? right : Math.max(8, rect.left - width - 8); article.style.left = `${left}px`; article.style.top = `${Math.max(8, Math.min(viewportHeight - height - 8, rect.top))}px`; }; article.style.cssText = "position:fixed;z-index:2147483647;width:min(360px,calc(100vw - 16px));max-height:min(480px,calc(100vh - 16px));overflow:auto;background:white;border:4px solid #28c4c2;border-radius:10px;padding:14px;box-shadow:0 8px 28px #0006"; shadow.append(article); position(); ownerDocument.defaultView?.addEventListener("scroll", position, true); ownerDocument.defaultView?.addEventListener("resize", position); const observer = new MutationObserver(() => { if (!article.isConnected) { ownerDocument.defaultView?.removeEventListener("scroll", position, true); ownerDocument.defaultView?.removeEventListener("resize", position); observer.disconnect(); } }); observer.observe(shadow, { childList: true }); article.focus(); }
