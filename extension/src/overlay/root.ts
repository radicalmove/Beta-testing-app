    ...
    setPageComments(comments) {
      ...
      for (const [commentIndex, comment] of comments.entries()) {
        const openThread = (marker?: HTMLElement) => {
          ...
          if (comment.capabilities.can_change_status && options.changeStatus) { const target = comment.status === "resolved" ? "open" : "resolved"; const statusButton = ownerDocument.createElement("button"); statusButton.type = "button"; statusButton.textContent = target === "resolved" ? "✓ Resolve" : "↶ Reopen"; statusButton.addEventListener("click", async () => { statusButton.disabled = true; try { await options.changeStatus!(comment.id, target); article.remove(); } catch (error) { statusButton.disabled = false; const alert = ownerDocument.createElement("p"); alert.setAttribute("role", "alert"); alert.textContent = error instanceof Error ? error.message : "Could not update status"; article.append(alert); } }); article.append(statusButton); }
